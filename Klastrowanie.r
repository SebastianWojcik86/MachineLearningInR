...
for(k in 1:9)
{
  iris_kmeans <- kmeans(x = iris[,-5],centers = (k),iter.max = 10,nstart = 10)
  ratio[k] <- iris_kmeans$tot.withinss/iris_kmeans$totss
  print(ratio[k])
}  
ratio
plot(y = ratio,x=1:9,type="o",pch=16,xlab="k",ylab='WSS / TSS',ylim = c(0,1),lwd=1.5)
abline(a=0.2,b=0,lty='dashed',col='red',lwd=1.5)
