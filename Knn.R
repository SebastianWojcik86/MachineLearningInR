...
model_kNN <- knn3(x = iris[,-5], # macierz zmiennych objaśniających
                  y = iris[,5],  # zmienna celu typu factor
                  k=3            # liczba sąsiadów
                  )
...
model_kNN <- knnreg(dist~speed,
                    data=cars,#zmienne podaje się przez formułę, a nie prez x,y
                    k = 3) 

predict(model_kNN,newdata = cars)
predict(model_kNN,newdata = data.frame(speed=13))

# porównajmy wartości rzeczywiste i teoretyczne
plot(cars$dist,predict(model_kNN,newdata = cars),ylim=range(cars$dist))
abline(0,1)

library(ModelMetrics)
predicted.dist=predict(model_kNN,newdata = cars)
mae(cars$dist,predicted.dist)
rmse(cars$dist,predicted.dist)

mean(abs(cars$dist-predicted.dist)) #MAE
mean(abs(cars$dist-predicted.dist)/cars$dist) #MAPE
rmse(cars$dist,predicted.dist)/mean(cars$dist) # CV
